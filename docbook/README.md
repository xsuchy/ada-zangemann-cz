...
- More refinement is necessary on the `<section>` elements for the page numbers. It results in a table of contents per chapter and could be used for page number on Scribus. Also a DocBook section can contain a title and info element, which is not fitting for a page indicator.
