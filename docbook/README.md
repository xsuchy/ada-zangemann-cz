...
- Rather than using XML IDs, an XPATH expression would be a more dynamic way to select content from Scribus. As an XML ID needs to be unique, it can be difficult to put it at the right place when sections are dissapearing. An XPATH expression can be more selective. Unfortunately an XPATH expressoin from a variable [can only be used since XSLT version 2.1](https://stackoverflow.com/questions/4630023/dynamic-xpath-in-xslt). For earlier versions of XSLT a match process can be created that functions as an approaximation.
